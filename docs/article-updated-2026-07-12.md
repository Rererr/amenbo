...
- Shift_JISの青空文庫（1.2MB）を、振り仮名を除いた本文として現実的なトークン量でテキスト化できたのはamenbo（**7,992**）。Jina Readerも文字化けはしないが振り仮名込み**329,380**トークンを一括で返し、Playwright MCPは同ページに**45万トークン**のスナップショットを生成した
...
| ツール | 形態 | 選定理由 |
|---|---|---|
| [amenbo](https://github.com/Rererr/amenbo) | MCP (stdio) | 本記事の主役。二段フェッチ＋CJK特化抽出 |
| [mcp-server-fetch](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch) | MCP (uvx) | MCP公式リファレンス実装。デファクトの基準線 |
| [Jina Reader](https://jina.ai/reader/) (r.jina.ai) | HTTP API | URL→Markdown系で最も普及。キー不要で再現しやすい |
| [Playwright MCP](https://github.com/microsoft/playwright-mcp) | MCP (npx) | ブラウザレンダリング系の代表 |
| [PixelRAG](https://github.com/StarTrail-org/PixelRAG) の pixelshot | CLI | 2026年上半期の話題株。「HTML解析をやめて全部スクショで読む」思想 |
...
現行版（v0.2.0）で同じハーネス（[`bench/`](https://github.com/Rererr/amenbo/tree/main/bench)）を実行し直した結果です。

- **7URLのトークン数はほぼ2026-07-10と同一**（amenbo・mcp-fetch・pixelshotは全一致、CSVの5,000倍差 amenbo 909 / Jina 4,593,027 もそのまま再現）。退行も水増しもありません。
- **1点だけ変化**: 青空文庫（Shift_JIS）に対する Jina Reader が、以前の 503 エラーから **329,380トークンで成功**に変わりました。文字化けはしませんが振り仮名を残したまま全文を一括で返すため、本文の可読性とトークン量では依然 amenbo（7,992・振り仮名除去・25ページ分割）が有利です。上表・本文はこの結果を反映済みです。
- **段階開示（outline）の効き**を数値化しました。同じ7ページ（英語ページ2本を含む）で `mode: outline` は `markdown` 比 **75〜97%** 少ないトークンでした（例: 都道府県一覧 5,337→659 で 87.7%減、青空文庫 7,992→408 で 94.9%減）。「読む前に読む量を決める」段階開示のコスト削減がそのまま出ています。
...
- **遮断サイト（err）の挙動が変わりました**。7月は接続リセットで即時失敗でしたが、現在は素のHTTPクライアントに対して応答を返さず放置します。amenboは「応答なし」と明示してブラウザ経由（`mode: screenshot`）の再試行を案内するよう改善しました。ブラウザ実体を持つJina Reader（12,028トークン）・Playwright MCPは取得に成功します。
- **amenbo側の改善**: alt無し画像と本文を持たないリンク（Zennの見出しアンカー `[](#...)`、Wikipediaの国旗アイコン・サムネイルリンク）をMarkdownに出さないようにしました。同一ページで zenn 5,104→4,527（-11%）、wiki 6,991→6,112（-13%）です。Wikipediaは7月以降サムネイルURLにutmパラメータが付くようになり、mcp-server-fetch・Jinaではその分のトークンが増えています。
- pixelshotは本環境でcdpバックエンドが出力を生成しなくなったため未再計測です（画像換算のため推定器の影響は受けません。上表の値を据え置きます）。

## 参考リンク

- [PixelRAG: Web Screenshots Beat Text for Retrieval-Augmented Generation (arXiv:2606.28344)](https://arxiv.org/abs/2606.28344)
